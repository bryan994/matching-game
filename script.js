...
class GamePlay {
    ...
    new_deck() { // create new deck
        ...
        const full_deck = this.shuffle_deck();
        full_deck.forEach((val, index) => {
            const card = document.getElementById(`${index+1}`);
            card.querySelector('.fal').setAttribute('class', `fal ${val['symbol']}`);
        });
    }

    create_deck() {
        const icon = [
            'pegasus','paw-claws','dragon','bat','jack-o-lantern','heart','leaf-maple','smile','snowflake','stars','ad','chess','chess-king-alt',
            'cactus','globe-asia','island-tropical','usd-circle','swords','thumbs-up','piggy-bank','peace','yin-yang','grin-hearts','grin-wink',
            'user-astronaut','futbol','trophy-alt','galaxy','planet-ringed','axe-battle'
        ]
        let icon_index = [];
        let count = 0;
        while(count < 16) { // random choose 8 different icons with combination of 16  
            let icon_count = Math.floor(Math.random() * icon.length);
            if(!icon_index.includes(icon_count)) {
                icon_index.push(icon_count);
                this.deck[count] = {symbol: 'fa-'+icon[icon_count], faceup: false, matched: false};
                this.deck[count+1] = {symbol: 'fa-'+icon[icon_count], faceup: false, matched: false};
                count += 2;
            }
        }
    }
    shuffle_deck() { // shuffle deck
        this.create_deck();
        let card_deck = this.deck;
        let current_index = card_deck.length;
        let temporary_value;
        let random_index;
    ...
    turn_up(selected_card_index) {
        const selectedCard = document.getElementById(`${selected_card_index}`);
        selectedCard.setAttribute('class', 'card-game__item card-game__item--active');
    }
    turn_down(selected_card_index) {
        const selectedCard = document.getElementById(`${selected_card_index}`);
        selectedCard.setAttribute('class', 'card-game__item');
    }

    turn(selected_card_index) {
        ...
        if (this.match_count >= MATCH_LIMIT) {
            ...
            let desc = this.game_time_seconds + ' seconds';
    ...
    // Check is icon match
    icon_match(first_card, second_card) {
    ...
    icon_is_match(first_card, second_card) {
        let card = $('#' + first_card + ', #' + second_card);
        card.addClass('card-game__item--match');
    }

    pair_match(first_card_index, second_card_index) {
        this.match_count += 1;
        this.flip_count = 0;
    ...
    async pair_not_matched(first_card_index, second_card_index) {
    ...
    show_new_time(game_timer) {
        game_timer.game_time_seconds += 1;
        let minutes = game_timer.game_time_minutes;
        ...
        let seconds = ("0" + game_timer.game_time_seconds).slice(-2);
